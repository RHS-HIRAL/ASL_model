...
def process_split(json_file, out_landmarks, out_labels):
    ...
    # Select 10% subset
    random.seed(42)
    subset_size = int(len(samples) * 0.1)
    ...
    hash_file = open(landmarks_path + ".hashes", 'a')  # For appending new hashes
    ...
    with mp_hands.Hands(static_image_mode=False, max_num_hands=2, min_detection_confidence=0.5) as hands:
        for sample_idx, sample in enumerate(subset_samples):
            ...
            if sample_hash in processed_hashes:
                print(f"[{sample_idx+1}/{total_samples}] Sample already processed, skipping.")
                continue

            print(f"\n[Sample {sample_idx+1}/{total_samples}]")
            print(f"Processing sample: URL={sample['url']}, Clip={sample['start_time']}s to {sample['end_time']}s, Label={sample['label']}")
            url = sample['url']
            ...
            temp_filename = f"temp_video_{int(time.time())}_{random.randint(1000, 9999)}.mp4"
            ...
            try:
                ...
                print(f"Sample processed. Landmark shape: {np.array(clip_landmarks).shape}")
    ...
    if batch_features:
        ...
        for h in batch_hashes:
            hash_file.write(h + '\n')
        hash_file.flush()
        print(f"Final checkpoint saved at {len(features)} samples.")

    hash_file.close()

    # Summary
    print("\n--- Processing Complete ---")
    print(f"Total attempted: {total_samples}")
